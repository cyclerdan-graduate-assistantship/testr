...
#         self.test_input_from_cli = False
        self.home_selection = 1
        self.working_directory = args[1]
        self.source_file_name = args[2]
        self.alternate_source_file_names = self.splitAlternateFileNames(args[3])
        self.test_file_name = args[4]
        if self.test_file_name == "":
            self.use_source_file_as_test_file = True
        self.test_cases = self.splitTestCases(args[5])
        self.test_input_from_cli = False
        if args[6].lower()[:1] == "y":
