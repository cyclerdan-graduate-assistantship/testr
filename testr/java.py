...
class Java(object):
    ...
    def compile(cls, file_path):
        ...
        res = ShellOutput(output='', execution_was_successful=True)
        return res
    ...
    def run(cls, run_file_path, get_input_from_command_line=False, input_file_path=None):
        ...
        #     shell_command = ['java -classpath ' + run_file_directory_path + ' ' + run_file_name]
        ...
        #         shell_command = 'java -classpath ' + run_file_directory_path + '/ ' + run_file_name + ' ' + File().get_text_from_file(input_file_path)
        #         print(shell_command[0])
        #         run_result = Shell.execute_shell_command(shell_command, True)
        #     else:
        #         shell_command = 'java -classpath ' + run_file_directory_path + '/ ' + run_file_name + ' < ' + input_file_path + ' > ' + run_file_directory_path + '/out.txt'
        #         print(shell_command[0])
        #         run_result = Shell.execute_shell_command(shell_command, True)
        #         run_result.output = File().get_text_from_file(run_file_directory_path + 'out.txt')
        # if run_result.output=='The filename, directory name, or volume label syntax is incorrect.\n':
        #     run_result.output=''
        run_result = ShellOutput(output='', execution_was_successful=True)
        run_result.output = ''
        run_result.execution_was_successful = True
        return run_result
